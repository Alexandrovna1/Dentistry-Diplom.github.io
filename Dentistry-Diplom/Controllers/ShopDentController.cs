...
namespace Dentistry_Diplom.Controllers
{
    public class ShopDentController : Controller
    {
        ...
        {
            DentistryInfo item = db.DentistryTable.Find(Id); //найдем добавляемая запись
            if (item != null)
                _shopDent.AddToDent(item); //добавим в прием

            return RedirectToAction("Index"); //перейдем в оформление приема
        }
